...
3. Select the **CDC_PROD** for the database, **ANALYTICS** for the schema, and **CDC_DS_WH** for the both the query and notebook warehouses. This Notebook includes the SQL scripts needed to create the destination database for table replication of the PostgreSQL tables into Snowflake and monitor the replication process.
